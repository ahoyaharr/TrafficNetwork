...
def generate_path(network, section1=None, section2=None, min_path_length=30000, max_path_length=75000,
                  max_offset_distance=500, random_distance=True, offset_heading=10, heading_noise=5,
                  omit_factor=1):
    """
    ...
    :param random_distance: if True, offset distances will be randomized.
    :param offset_heading: the number of degrees that a point's heading will be offset from the true heading.
    :param heading_noise: if
    ...
    def random_vertex(exclude=-1):
        return sample(vertices - {exclude}, 1)[0]
    ...
    section2 = graph_tool.util.find_vertex(network.graph, network.node_id, section2) if section2 else random_vertex(
        section1)
    ...
    true_data = [DataPoint(timestamp, network.node_speed_limit[v], network.node_locations[v][0],
    ...
    offset_data = [DataPoint(timestamp, network.node_speed_limit[v],
                             *Point.as_tuple(
                                 offset_point(Point(network.node_locations[v][0],
                                                    network.node_locations[v][1],
                                                    network.node_heading[v]),
                                              (randrange(max_offset_distance) if random_distance
                                               else max_offset_distance), randrange(359)))[:2],
                             (network.node_heading[v] + offset_heading + randrange(-heading_noise,
                                                                                   heading_noise) % 360))
...
def multiple_artificial_paths(network, num_trips, section1=None, section2=None, min_path_length=30000,
                              max_path_length=50000, max_offset_distance=200, random_distance=True,
                              offset_heading=10, heading_noise=5, omit_factor=1):
    """
    ...
    for i in range(num_trips):
        offset, true = generate_path(network, section1, section2, min_path_length, max_path_length, max_offset_distance,
                                     random_distance, offset_heading, heading_noise, omit_factor)
